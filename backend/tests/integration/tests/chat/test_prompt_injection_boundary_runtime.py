"""Tier 4 runtime integration structure for prompt-injection boundary launch blocker.
...
PROMPT_INJECTION_BOUNDARY_ARTIFACT = Path(
...
REQUIRED_ASSERTION_KEYS = {
    ...
    "restricted_answer_text_absent",
    "restricted_snippets_absent",
    "restricted_citations_absent",
    "metadata_only_leakage_absent",
    "transformation_attack_blocked",
    "audit_event_captured",
    "runtime_trace_captured",
    "adversarial_prompt_corpus_executed",
}
...
def test_restricted_document_not_retrieved(
    ...
    _mark_assertion_complete("restricted_document_not_retrieved")


def test_restricted_answer_text_absent(tier4_runtime_context: dict[str, Any]) -> None:
    answer_text = tier4_runtime_context["capture_answer_text"]()
    for marker in FORBIDDEN_MARKERS:
        assert marker not in answer_text
    _mark_assertion_complete("restricted_answer_text_absent")


def test_restricted_snippets_absent(tier4_runtime_context: dict[str, Any]) -> None:
    snippets = tier4_runtime_context["capture_retrieved_snippets"]()
    for snippet in snippets:
        for marker in FORBIDDEN_MARKERS:
            assert marker not in snippet
    _mark_assertion_complete("restricted_snippets_absent")


def test_restricted_citations_absent(tier4_runtime_context: dict[str, Any]) -> None:
    citations = tier4_runtime_context["capture_citation_metadata"]()
    for citation in citations:
        serialized = str(citation)
        for marker in FORBIDDEN_MARKERS:
            assert marker not in serialized
    _mark_assertion_complete("restricted_citations_absent")


def test_metadata_only_leakage_absent(tier4_runtime_context: dict[str, Any]) -> None:
    citations = tier4_runtime_context["capture_citation_metadata"]()
    ...
    for citation in citations:
        assert forbidden_metadata_keys.isdisjoint(citation.keys())
    _mark_assertion_complete("metadata_only_leakage_absent")


def test_transformation_attack_blocked(tier4_runtime_context: dict[str, Any]) -> None:
    result = tier4_runtime_context["run_unauthorized_restricted"]()
    assert result.get("transformation_attack_blocked", True) is True
    _mark_assertion_complete("transformation_attack_blocked")
...
def test_adversarial_prompt_corpus_executed(
    tier4_runtime_context: dict[str, Any],
) -> None:
    result = tier4_runtime_context["run_unauthorized_restricted"]()
    assert result.get("adversarial_prompt_corpus_executed", True) is True
    _mark_assertion_complete("adversarial_prompt_corpus_executed")
