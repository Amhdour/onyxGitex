...
## Content
- In scope: Onyx-based RAG assistant, identity context propagation, retrieval authorization logic, tool/MCP interfaces, logging and telemetry plans.
- Out of scope: non-Onyx legacy search systems, external customer-facing chat products.
- Environments: local repo analysis and planned staging verification only.
- Decision: scope is sufficient for first launch-readiness draft, not for production attestation.
