...
| ToolRuntimeWiringAdapter | Tier 3 | Runtime-adjacent tool runtime wiring adapter evidence PASS and main llm_loop.py runtime-adjacent wiring evidence PASS; production full runtime wiring remains NOT_PASS | `backend/onyx/security_readiness/tool_runtime_wiring_adapter.py`; `security-readiness/evidence-artifacts/test-results/tool-runtime-wiring-adjacent-tests.json`; `security-readiness/evidence-artifacts/test-results/tool-runtime-wiring-main-llm-loop-tests.json`; `security-readiness/final-control-status-canonical.md` | Full runtime wiring proof in `llm_loop.py` and research-agent execution paths, plus full tool runtime authorization evidence | Non-critical Tier 3 signal only; RAG_PLUS_TOOLS remains NOT_READY and launch stays NOT_ENOUGH_EVIDENCE |
